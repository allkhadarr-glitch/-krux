  ...
  async function sendInvite(e: React.FormEvent) {
    ...
    try {
      const r = await fetch('/api/invites', {
        ...
        body: JSON.stringify({ email: inviteEmail, phone: invitePhone.trim() || undefined, role: inviteRole }),
      })
            ...
            <form onSubmit={sendInvite} className="space-y-3">
              <div className="flex gap-3 items-end flex-wrap">
                ...
                <div className="flex-1 min-w-[140px]">
                  <label className="text-xs font-semibold text-[#64748B] uppercase tracking-wide">Phone <span className="normal-case text-[#334155]">(optional — for WhatsApp)</span></label>
                  <input value={invitePhone} onChange={(e) => setInvitePhone(e.target.value)} type="tel"
                    className="w-full mt-1 bg-[#0A1628] border border-[#1E3A5F] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#00C896]"
