      ...
      <div className="w-full max-w-lg">
        ...
        <div className="mb-5 flex items-start justify-between">
          ...
          <div className="text-right">
            <p className="text-[#334155] text-[10px] uppercase tracking-wide">Entity record</p>
            <p className="text-[#1E3A5F] text-[10px] mt-0.5">Retrieved {retrievedAt}</p>
          </div>
        </div>
        ...
        <div className="bg-[#0A1628] border border-[#1E3A5F]">
          ...
          <div className="px-6 py-5 border-b border-[#1E3A5F]">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-[9px] text-[#334155] uppercase tracking-[0.2em] mb-2">
                  KTIN — KRUX Trade Identity Number
                </p>
                <p className="text-3xl font-black text-white tracking-wider">{entity.krux_id}</p>
                <p className="text-[10px] text-[#334155] mt-2">Issued {issueDate}</p>
              </div>
              {entity.is_verified && (
                <div className="flex items-center gap-1.5 border border-[#00C896]/30 px-2.5 py-1 mt-1">
                  ...
                  <span className="text-[9px] font-black text-[#00C896] tracking-[0.2em]">VERIFIED</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="px-6 py-4 border-b border-[#1E3A5F] grid grid-cols-2 gap-x-8 gap-y-4">
            <div>
              <p className="text-[9px] text-[#334155] uppercase tracking-[0.15em] mb-1.5">Registered Name</p>
            ...
            <div>
              <p className="text-[9px] text-[#334155] uppercase tracking-[0.15em] mb-1.5">Entity Classification</p>
              <p className="text-sm font-semibold text-white leading-tight">
                {TYPE_LABEL[entity.entity_type] ?? entity.entity_type}
              </p>
              <p className="text-[10px] text-[#334155] mt-0.5">{entity.country_code} — East Africa</p>
            </div>
          </div>

          {/* Compliance record */}
          <div className="px-6 py-5 border-b border-[#1E3A5F]">
            <p className="text-[9px] text-[#334155] uppercase tracking-[0.15em] mb-4">Compliance Record</p>
              ...
              <div className={`border ${tier.border}`}>
                <div className="px-4 py-3 flex items-start justify-between border-b border-[#1E3A5F]">
                  <div>
                    ...
                    <p className="text-[10px] text-[#64748B] mt-0.5">{tier.description}</p>
                  </div>
                  <div className="text-right">
                    <p className={`text-3xl font-black ${tier.color}`}>{entity.compliance_score}</p>
                    <p className="text-[9px] text-[#334155]">/ 100</p>
                  </div>
                </div>
                <div className="px-4 py-3 grid grid-cols-3 gap-3">
                  <div>
                    <p className="text-[9px] text-[#334155] uppercase tracking-wide mb-1">Shipments</p>
                    <p className={`text-base font-black ${tier.color}`}>{entity.total_shipments}</p>
                  </div>
                  <div>
                    <p className="text-[9px] text-[#334155] uppercase tracking-wide mb-1">Cleared</p>
                    <p className={`text-base font-black ${tier.color}`}>{entity.cleared_on_time}</p>
                  </div>
                  {entity.avg_clearance_days && (
                    <div>
                      <p className="text-[9px] text-[#334155] uppercase tracking-wide mb-1">Avg days</p>
          ...
          <div className="px-6 py-3 flex items-center justify-between">
            <div className="space-y-0.5">
              <Link
                ...
                className="no-print text-[9px] text-[#64748B] hover:text-[#00C896] uppercase tracking-wide transition-colors block"
              >
                How scores are calculated
              </Link>
              {entity.last_shipment_at && (
                <p className="text-[9px] text-[#1E3A5F]">
            ...
            <div className="flex items-center gap-2">
              ...
              <a
                href={`${appUrl}/signup`}
                className="no-print text-[9px] px-3 py-1.5 border border-[#00C896]/40 text-[#00C896] hover:bg-[#00C896]/10 transition-colors uppercase tracking-widest"
              >
                Register entity
              </a>
            </div>
        ...
        {/* Document footer */}
        <p className="text-[9px] text-[#1E3A5F] mt-4 leading-relaxed">
